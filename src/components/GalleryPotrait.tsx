...
const GalleryPotrait = ({potrait:{size,img,name,tags},onHover,onLeave,overlay}:PropTypes) => {
    return (
        <div className={`transition duration-300 space-y-2 ${overlay && "grayscale opacity-25"}`}>
            <img alt='' onMouseEnter={onHover} onMouseLeave={onLeave} className={`transition duration-500 ${size==="sm"&&"min-h-[175px]"} ${size==="md"&&"min-h-[280px]"} ${size==="lg"&&"min-h-[400px]"} object-cover object-center rounded-lg hover:scale-[0.98]`} src={img} />
